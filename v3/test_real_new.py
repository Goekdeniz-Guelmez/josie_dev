        ...
        while self.running:
            try:
                ...
                with torch.no_grad():
                    ...
                    # print(f"Semantic tokens unique values: {torch.unique(semantic_tokens).tolist()}")
                    # print(f"Acoustic tokens unique values: {torch.unique(acoustic_tokens).tolist()}")
                    print(f"Combined tokens: {combined_tokens}")
                    
                    # Process through decoder
                    output_waveform = self.model.decode(self.reasoner_output) # torch.Size([1, 1, 8000])
