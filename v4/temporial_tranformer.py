...
class TemporalTransformer(nn.Module):
    ...
    def forward(
            ...
            semantic_tokens: torch.Tensor, # [B, L] 
            acoustic_tokens: torch.Tensor # [B, L]
    ) -> torch.tensor:

        # Get embeddings
        text_emb = self.text_embedding(text_tokens) # [B, L1, H]
        semantic_emb = self.semantic_embedding(semantic_tokens) # [B, L2, H]
        acoustic_emb = self.acoustic_embedding(acoustic_tokens) # [B, L3, H]
