...
class RealTimeSpeechSystem:
    ...
    def process_audio(self):
        buffer = []
        buffer_size = 4  # Process 1 second of audio at a time
        ...
        while self.is_running:
            ...
            if len(buffer) >= buffer_size:
                ...
                # Process with model or simulate for testing
                if self.model is not None:
                    ...
                    print("\nSimulating JOSIE model output...")
                    # Just add some effects to the input audio
                    response = audio_data * 0.8  # Reduce volume
                    
                    # Add echo effect
                    echo_delay = int(0.2 * self.sample_rate)  # 200ms delay
                    echo = np.zeros_like(response)
                    echo[echo_delay:] = response[:-echo_delay] * 0.5
                    response = response + echo
                    
                    # Normalize to prevent clipping
                    response = response / max(1.0, np.max(np.abs(response)))
